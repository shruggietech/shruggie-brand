...
- **Source authority:** `brand.json` declares authoritative logo mode. Full binds to `full-mark-master`; Reduced binds to `reduced-mark-master`. Their current SHA-256 values, mask methods, and allowed transformations are mandatory. Generated marks and lockups must appear in `logos/provenance.json`, and any later source, binding, hash, mask, or geometry change requires fresh owner approval.
