...
- **Source authority:** `brand.json` declares authoritative logo mode. Full binds to `full-mark-master`; Reduced binds to `reduced-mark-master`. Their current SHA-256 values, owner-approved mask methods, mask approval identities and dates, and allowed transformations are mandatory. Generated marks and lockups must appear in `logos/provenance.json`, and every raster and platform artifact is verified against its declared master. Any later source, binding, hash, mask, or geometry change requires fresh owner approval.
