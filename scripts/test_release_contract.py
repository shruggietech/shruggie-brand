...
class ReleaseContractTests(unittest.TestCase):
    ...
    def test_repository_metadata_and_notes_agree_for_2_2_1(self):
        metadata = release_contract.load_metadata(ROOT, "2.2.1")
        notes = release_contract.render_notes(metadata)

        self.assertEqual(metadata["skill_version"], "2.2.1")
        self.assertEqual(metadata["canon_version"], "1.5.0")
        self.assertEqual(metadata["site_version"], "2.2.1")
        self.assertEqual(release_contract.current_version(ROOT), "2.2.1")
        self.assertIn("Skill version: `2.2.1`", notes)
        self.assertIn("Canon version: `1.5.0`", notes)
        self.assertIn("Existing kits need migration: **no**", notes)
        self.assertIn("dark backgrounds", notes)
    ...
    def test_expected_assets_are_exact_and_use_embedded_brand_versions(self):
        ...
        self.assertEqual(set(release_contract.expected_assets(metadata)), {
            "shruggie-brandbuilder-2.2.1.skill",
            "shruggie-brandbuilder-2.2.1-portable.zip",
            "shruggietech-brand-1.0.0-bb2.2.1.zip",
            "fragcap-brand-1.1.1-bb2.2.1.zip",
            "go-schedule-brand-1.0.0-bb2.2.1.zip",
            "glitchpad-brand-1.1.1-bb2.2.1.zip",
            "covarity-brand-1.0.0-bb2.2.1.zip",
            "eso-weave-brand-1.0.2-bb2.2.1.zip",
            "cueson-brand-1.0.1-bb2.2.1.zip",
        })
        self.assertEqual(
    ...
    def test_site_package_version_must_match_release(self):
        ...
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(
                ValueError, "site package version 1.1.2 does not match release 2.2.1"
            ):
                release_contract.load_metadata(ROOT, "2.2.1")
    ...
    def test_compiler_release_and_brand_canon_versions_can_diverge(self):
        ...
        with mock.patch.object(
            ...
            metadata = release_contract.load_metadata(ROOT, "2.2.1")
            self.assertEqual("2.2.1", metadata["skill_version"])
            self.assertEqual("1.2.0", metadata["canon_version"])
            self.assertEqual("2.2.1", release_contract.current_version(ROOT))
