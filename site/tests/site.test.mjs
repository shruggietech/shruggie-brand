...
if (!brandPortfolioSource.includes("'--brand-portfolio-surface': brand.portfolioSurface") || brandPortfolioSource.includes('data-showcase-surface=')) throw new Error('portfolio is not isolated from brand-page showcase surfaces');
...
if (!globalStyles.includes(".guideline-layout[data-guide-mode='light']") || globalStyles.includes(".brand-card[data-showcase-surface='governed-light']")) throw new Error('brand light mode is missing or still applies to homepage cards');
...
if (!/\.brand-card h3 \{[^}]*color: #fff;/s.test(globalStyles) || !/\.brand-card-description \{[^}]*color: #fff;/s.test(globalStyles) || !/\.brand-actions a \{[^}]*color: #fff;/s.test(globalStyles)) throw new Error('portfolio dark cards have lost their white text roles');
if ((globalStyles.match(/background: var\(--brand-portfolio-surface\)/g) ?? []).length !== 2) throw new Error('desktop and mobile portfolio surfaces do not share the approved dark card source');
if (/\.brand-(?:card|accordion) \{[^}]*background:[^;}]*var\(--panel\)/s.test(globalStyles)) throw new Error('portfolio still inherits the site panel and becomes light in light mode');
...
if (!/\.brand-card \.brand-actions \{[^}]*visibility: hidden;/s.test(globalStyles) || !/\.brand-card:hover \.brand-actions, \.brand-card:focus-within \.brand-actions \{[^}]*visibility: visible;/s.test(globalStyles)) throw new Error('desktop action visibility does not prevent hidden keyboard targets');
if (!/\.brand-card:focus-visible \{[^}]*outline: 2px solid #fff;[^}]*box-shadow:/s.test(globalStyles) || !/\.brand-actions a:focus-visible \{[^}]*outline: 2px solid #fff;[^}]*box-shadow:/s.test(globalStyles)) throw new Error('portfolio keyboard entries lack the dual focus treatment');
if (!/\.brand-card-stage \{[^}]*min-height: 7rem;[^}]*padding-bottom: 1rem;/s.test(globalStyles)) throw new Error('desktop card stage does not reserve 16-pixel lower clearance for descriptions and actions');
if (!/@media \(prefers-reduced-motion: reduce\) \{[^}]*\.brand-actions a[^}]*transition: none;/s.test(globalStyles)) throw new Error('portfolio action controls lack an explicit reduced-motion override');
if (!brandPortfolioSource.includes('const hasThirdPartyProjects = brands.some((brand) => Boolean(brand.vendorBoundary));')) throw new Error('portfolio notice presence is not derived as a boolean applicability rule');
