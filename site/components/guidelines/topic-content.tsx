...
function Overview({ portal }: { portal: GuidelinePortal }) { const value = portal.content.overview; const contract = portal.implementation; const overrides = Object.entries(contract.rules.overrides); return <><section className="guide-section"><h2 id="brand-overview">Brand overview</h2><p>{portal.brand.descriptor}</p><p>{portal.brand.idea}</p></section><section className="guide-section"><h2 id="foundations">{value.foundation_title || 'Foundations'}</h2><p>{value.foundation || portal.brand.descriptor}</p></section><section className="guide-section"><h2 id="promises">Promises</h2><Items values={value.promises} /></section><section className="guide-section"><h2 id="boundaries">Boundaries</h2><div className="guide-columns"><div><h3>In scope</h3><Items values={value.in_scope} /></div><div><h3>Out of scope</h3><Items values={value.out_of_scope} /></div></div>{value.sharp_edge && <aside className="guide-notice"><strong>Sharp edge</strong><p>{value.sharp_edge}</p></aside>}</section><section className="guide-section contract-summary" aria-labelledby="generated-contract"><h2 id="generated-contract">Generated implementation contract</h2><p>{contract.hosted.scope}</p><p><strong>Affiliation:</strong> {portal.brand.affiliation || 'No parent affiliation declared.'}</p><p><strong>Inheritance:</strong> {contract.rules.inheritance}</p><h3>Declared interface overrides</h3>{overrides.length ? <ul>{overrides.map(([name, value]) => <li key={name}><code>{name}</code>: <code>{value}</code></li>)}</ul> : <p>None.</p>}<dl className="metric-list">{Object.entries(contract.versions).map(([name, version]) => <div key={name}><dt>{name.replaceAll('_', ' ')}</dt><dd>{version}</dd></div>)}</dl><h3>Bindings</h3><ul>{Object.entries(contract.bindings).map(([name, path]) => <li key={name}><strong>{name.replaceAll('_', ' ')}:</strong> <code>{path}</code></li>)}</ul><a className="guide-source-download" href={contract.hosted.manual_path}>Open the BrandBuilder system manual</a></section><section className="guide-section"><h2 id="built-to-ship">Built to ship.</h2><p>Version {portal.brand.version} includes production assets, standards, typography specimens, and an installable shadcn registry.</p><a className="guide-source-download" href={`/${portal.brand.slug}/brand/r/registry.json`}>Open registry</a></section></>; }
