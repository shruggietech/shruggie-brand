...
export default async function Page({ params }: { params: Promise<{ slug?: string[] }> }) { const page = source.getPage((await params).slug); if (!page) notFound(); const route = routeByPath(page.url); const MDX = page.data.body; return <DocsPage id="content" className="docs-page" toc={page.data.toc} full={page.data.full} footer={{ className: 'docs-pagination' }}><StructuredData route={route} /><DocumentationNoScriptNav currentPath={route.pathname} /><DocsTitle>{page.data.title}</DocsTitle><DocsDescription>{page.data.description}</DocsDescription><DocsBody><MDX components={getMDXComponents()} /></DocsBody><Footer /></DocsPage>; }
