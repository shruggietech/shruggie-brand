...
export function guidelineSections(portal: GuidelinePortal): GuidelineSection[] {
  ...
  for (const topic of portal.topics) {
    ...
    } else if (topic.section === topic.label) {
      sections.push({ label: topic.section, destination: topic });
    } else {
      sections.push({ label: topic.section, children: [topic] });
    }
  }
  return sections;
}
...
export function guidelineTree(portal: GuidelinePortal): Root {
  return {
    ...
    children: guidelineSections(portal).map((section) => section.destination
      ? { type: 'page' as const, name: section.destination.label, url: section.destination.path }
      : { type: 'folder' as const, name: section.label, defaultOpen: true, collapsible: true, children: (section.children ?? []).sort((left, right) => left.order - right.order).map((topic) => ({ type: 'page' as const, name: topic.label, url: topic.path })) }),
  };
}
