...
# -*- coding: utf-8 -*-
"""
gen_nextjs.py: emit the Next.js binding layer for a contracted brand.
...
def emit_globals(canon, brand, dark, light):
    ...
    for k, v in R.items():
        if k != "--radius":
            L.append("  %s: %s;" % (k, v))
    L.append("")
    L.append("  --font-sans: var(--font-body);")
    L.append("  --font-mono: var(--font-mono-role);")
    L.append("  --font-display: var(--font-display-role);")
    L.append("}")
    L.append("")
...
def emit_theme_item(canon, brand, dark, light):
    ...
    theme.update({"font-display": "var(--font-display-role)"})
    return {
...
def emit_fonts_ts(brand):
    ...
    lines = ["// GENERATED by shruggie-brandbuilder. Keep this file in nextjs/ and the",
             "// bundled font tree in fonts/ so every path remains local and deterministic.",
             'import localFont from "next/font/local"', ""]
    exports = {"display": "displayFont", "body": "bodyFont", "mono": "monoFont"}
    variables = {"display": "--font-display-role", "body": "--font-body", "mono": "--font-mono-role"}
    for role in ("display", "body", "mono"):
        ...
        for face in all_faces:
            ...
            if key not in preferred or face["format"] == "woff2":
                preferred[key] = face
        selected = [preferred[key] for key in sorted(preferred)]
        lines += ["export const %s = localFont({" % exports[role], "  src: ["]
        for face in selected:
            lines.append('    { path: "../%s", weight: "%d", style: "%s" },' % (face["path"], face["weight"], face["style"]))
        lines += ["  ],", '  variable: "%s",' % variables[role], '  display: "swap",', "})", ""]
    lines.append("// Approved weights: %s" % "; ".join("%s %s" % (families[role]["name"], "/".join(str(weight) for weight in families[role]["weights"])) for role in ("display", "body", "mono")))
    lines += ["export const fontVariables = [", "  displayFont.variable,", "  bodyFont.variable,", "  monoFont.variable,", '].join(" ")', ""]
    return "\n".join(lines)
...
def main():
    ...
    w(os.path.join(nd, "fonts.ts"), emit_fonts_ts(brand))
