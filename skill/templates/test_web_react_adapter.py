...
class WebReactAdapterTests(unittest.TestCase):
    ...
    def test_output_is_deterministic_complete_and_server_safe(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual("1.1.0", manifest["adapter_version"])
            ...
            self.assertEqual("web/react/environment.tsx", manifest["entries"]["environment"])
            self.assertEqual(["AppFrameEnvironmentBridge", "measureImeBlockEnd"], manifest["environment_exports"])
            ...
            self.assertIn('layout = "contained"', server)
            self.assertIn('layout?: "contained" | "full-bleed"', server)
            self.assertIn('data-bb-layout={layout}', server)
            ...
            self.assertIn('export { AppFrameEnvironmentBridge, measureImeBlockEnd } from "./environment";', client)
            ...
            self.assertTrue(environment.startswith('"use client";'))
            self.assertNotIn("radix-ui", environment)
            self.assertNotIn("react-dom", environment)
            self.assertIn("window.innerHeight - viewport.height - viewport.offsetTop", environment)
            self.assertIn("layoutBaseline - window.innerHeight", environment)
            self.assertIn("measureImeBlockEnd(viewport, baseline)", environment)
            self.assertNotIn("viewport ? 0 :", environment)
            self.assertIn('viewport?.addEventListener("scroll", update)', environment)
            self.assertIn('root.dataset.bbAppFrameMounted = "true"', environment)
            self.assertIn("resizedLayoutInset", environment)
            self.assertIn("delete root.dataset.bbAppFrameMounted", environment)
    ...
    def test_tokens_are_react_free_and_component_css_uses_semantic_namespace(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn('.bb-app-frame[data-bb-layout="full-bleed"] .bb-app-frame__scroll {', css)
            self.assertIn('grid-template-rows: minmax(0, 1fr); overflow: hidden;', css)
            self.assertIn('.bb-app-frame[data-bb-layout="full-bleed"] .bb-app-frame__content', css)
