...
Use Author mode when the task changes Brand Canon, Interface Canon, identity source, a brand contract, generator, adapter, recipe, schema, or governed instruction. The task must already authorize the proposed mutation. Apply the repository's required specification workflow, identity approval, provenance, accessibility, publication, and release safeguards.
