...
The correction reviews on PR #26 found nine additional defects, tracked as issues #27 through #35. The merge and release remain held while the branch removes stale outputs during capability downgrades, separates ICO verification from SVG raster verification, keeps image-backed core masters independent of Pillow, includes Pillow compositing in the raster capability contract, clears stale image and PDF QC evidence, makes the repository's Markdown prose rule enforceable in CI, and executes regression tests on Python 3.8 rather than compiling them without running them.
