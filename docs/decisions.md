...
S003 revalidation found a dormant ImageMagick SVG branch in raster generation. Advertising and copying that branch into image QC would contradict the governed high-fidelity fallback chain in `skill/references/04-toolchain.md`. The probe, raster generation, and image QC therefore use only librsvg, resvg, Inkscape, or the verified Node resvg wrapper for SVG rendering. ImageMagick remains available solely for validated multi-entry ICO generation. The probe validates any `convert` executable as ImageMagick before recording ICO capability, and ICO generation reuses that recorded result instead of rediscovering an executable from `PATH`. This prevents the unrelated Windows filesystem conversion utility from producing a false positive or overriding the Pillow fallback.
...
### S005 guarded staging disposition

- The existing dated archive was reused instead of duplicated. All 2,479 archived files matched live content by path, byte count, and SHA-256; the only two live-only files were regenerated Python bytecode caches. A second large archive would have preserved no additional authoritative or historical content.
- The authenticated Cloudflare API integration satisfies the required infrastructure-access gate despite the absent local CLI and token environment variable. A successful account-scoped read was required, and operational identifiers remain private.
- Historical bulk artifacts, unrelated research, private session output, and the original operator directive moved to governed cold storage. S005 did not invent new locations in sibling repositories when ownership or collision-free placement was ambiguous.
