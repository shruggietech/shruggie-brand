...
## PR completion and post-merge CI correction (2026-09-23)

- PR #252 head `d69ac2a` passed approved-identity-proofs, Python 3.8 compatibility, verified-build, and aggregate build. The three first-round Codex threads were answered and resolved. The one requested second review completed with no findings and a thumbs-up; no separate security review was requested under the owner's two-round cap. The owner merged the PR as `f2d102a`.
- The first `main` Build run 35819083608 failed in the Windows canonical-host proof export with `canonical-host Gate 2 derivative manifest drift`; Python 3.8 passed and dependent verification skipped. The successful PR head and merge commit have identical trees, and the proof jobs used the same Windows runner image, Node version, and Python package versions. A single failed-job rerun passed the exact Gate 2 hash without a source change, confirming intermittent output rather than an approved-source edit. The rerun's complete verified-build remains in progress at this evidence update.
- Three focused tests first failed against the old exporter; a fourth destination-confinement case was added after implementation. All four pass after the bounded retry. The correction retries only a `DerivativeManifestDrift` once from a clean generated destination. A second mismatch still raises with both expected and actual hashes, and all other failures remain fail-fast. A failed proof job now uploads its generated evidence for forensic comparison. The approved ledger and logo/source files are unchanged.
