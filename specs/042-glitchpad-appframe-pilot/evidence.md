...
| Focused desktop resize correction | Glitchpad PR #197 second Codex review at `ac9765ef510dba98a4acddf7b2ebcad44a2ffe2a` | A valid P1 finding showed that retaining the previous layout height for every focused editable element could classify an ordinary Windows resize as IME obstruction. The generated bridge now preserves the layout-height baseline only when Chromium's mobile hint or a bounded Android, iOS, or iPadOS capability check identifies a mobile environment. VisualViewport geometry remains active on every host, while desktop layout resize resets to the current window height. The focused generator regression fails before and passes after the correction. |
...
- Modern engines use `:has()`, `100dvh`, and VisualViewport. The generated environment bridge also marks the mounted root, falls back to `100vh`, and combines VisualViewport and per-orientation window-height differentials on positively identified mobile environments so API 24 WebViews that resize both viewports still publish IME obstruction without treating focused desktop window resizes as keyboards.
