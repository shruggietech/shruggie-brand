...
| Vendored documentation portability | Glitchpad PR #197 `Documentation and public surface` at `681fe1b4804f6ec0195b717fcd48f5ad61e24d87` | The consumer link checker rejected the site-root `/docs/` target in generated `enforcement/IMPLEMENTATION.md`. The generator now preserves `/docs/` as the label while emitting the absolute hosted-manual URL, and the documentation-contract regression rejects the nonportable root-relative form. A replacement artifact is required before downstream host evidence can be final. |
| Mirrored VisualViewport correction | Glitchpad CI run `35470288882`, API 24 | Actual-host evidence measured a resized layout and visual viewport height of 370 pixels while the generated bridge still published a zero IME inset. The bridge now takes the maximum of the VisualViewport differential and its per-orientation layout-height differential instead of disabling the latter whenever `window.visualViewport` exists. Four focused adapter tests and a clean Glitchpad kit build pass locally; replacement actual-host evidence remains required. |
| Focused desktop resize correction | Glitchpad PR #197 second Codex review at `ac9765ef510dba98a4acddf7b2ebcad44a2ffe2a` | A valid P1 finding showed that retaining the previous layout height for every focused editable element could classify an ordinary Windows resize as IME obstruction. A follow-up P2 review rejected user-agent preference and OS-name inference. The generated bridge now preserves the layout-height baseline only for a coarse-pointer, no-hover input capability by default and exposes an explicit boolean override for hosts with stronger knowledge. VisualViewport geometry remains active on every host, while ordinary desktop layout resize resets to the current window height. The focused generator regression fails before and passes after the correction. |
...
- Modern engines use `:has()`, `100dvh`, and VisualViewport. The generated environment bridge also marks the mounted root, falls back to `100vh`, and combines VisualViewport and per-orientation window-height differentials for the declared coarse-pointer, no-hover capability (or an explicit host override) so API 24 WebViews that resize both viewports still publish IME obstruction without treating ordinary focused desktop window resizes as keyboards.
