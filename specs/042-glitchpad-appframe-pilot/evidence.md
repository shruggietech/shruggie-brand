...
- Modern engines use `:has()`, `100dvh`, and VisualViewport. The generated environment bridge also marks the mounted root, falls back to `100vh`, and combines VisualViewport and per-orientation window-height differentials so API 24 WebViews that resize both viewports still publish IME obstruction.
