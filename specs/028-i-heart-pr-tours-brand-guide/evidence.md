...
**State**: Gate 2 candidate `iheartpr-g2-r4` is approved. The owner has corrected the agent's private-publication assumption and explicitly authorized the complete public brand-website surface set. Pull-request correction and exact-head verification are in progress.
...
- The Python 3.8 regression now uses the repository UTF-8 writer, and the raster integration test skips only when no renderer is measured. Hosted canonical proof generation uses the exact approved Node 24.11.0 and resvg 2.6.2 toolchain instead of preferring an unrelated native renderer or floating the host runtime.
- Hosted heads `f0c2366`, `1940d32`, `2fb834a`, and `495adcc` proved the Python 3.8 fix and then isolated four environment differences: a floating Node version, operating-system-specific PNG output, a 16px proof whose entire visible mark legitimately falls inside the edge band, and Gate 2 manifest hashes that transitively included platform-specific PNG encoding inside eight monochrome SVG wrappers. Each affected run was rejected before publication. The final workflow retains Windows as the exact canonical host, exports its 32 hash-validated approved proofs and exact approved Gate 2 manifest as ephemeral CI evidence, and requires Linux to verify every canonical artifact hash before comparison. Proof checks retain unchanged topology, edge-band, bounds, centroid, and color tolerances. When no non-edge interior pixels exist, color is measured across the shared silhouette instead of producing an undefined result. Gate 2 comparison requires identical derivative paths, exact surrounding SVG bytes, and identical decoded RGBA dimensions and pixels for differently encoded embedded PNGs. One-channel pixel mutation tests fail. Exact Node 24.11.0 Linux-container reproductions passed both the complete IHPRT proof matrix and Gate 2 logo provenance against the Windows artifact. No generated proof raster or derivative is committed, and local canonical-host validation remains byte-exact.
- Final local review-fix validation passes 51 brand-contract tests, 17 icon-suite tests, 21 identity-continuity tests, 57 pipeline tests, 30 site-preparation tests, 5 package-release tests, 14 release-contract tests, 4 continuity-audit tests, and the 34-check glyph suite. All eight production kits rebuild with zero reported problems; IHPRT applies all 12 approved platform targets, retains 434 final manifest entries, and passes the light-ground PDF gate.
...
- Review fixes were published in commit `24619eeb67a34dc6853d4aacb8cae907b2309435`. The three second-round findings received individual responses at `discussion_r3990384344`, `discussion_r3990384541`, and `discussion_r3990384687`; all three threads are resolved. Together with the five resolved first-round threads, the pull request has eight review threads and zero unresolved threads.
- GitHub reported the manual second review as completed against `38246eb0fb13256ea8028d0e9b5c6eec151a2acb`. No separate security-bot comment, review, PR-body reaction, or review-trigger reaction was present when the review ledger was captured. No third review was requested.
- Hosted run `34621119048` passed canonical approval export, Python 3.8 compatibility, the full geometry suite, all eight kit builds, release certification, and generated-contract checks, then exposed one final consumer mismatch during static-site preparation: `public_showcase(..., kit)` repeated the exact manifest-byte check after `verify.py` had already accepted the Linux derivatives against the exact Windows approval artifact by decoded-pixel equivalence. The kit-aware approval ledger now reuses that same fail-closed portable comparison whenever exact bytes differ, and the site-export step receives the canonical artifact root. A focused regression proves a matching canonical manifest with an equivalent local inventory remains private; the 52 brand-contract tests, 58 pipeline tests, 30 site-preparation tests, and Markdown policy check pass locally. This changes no identity bytes, approval hashes, public surfaces, or comparison tolerances.

## Owner correction: mandatory website publication

**Recorded**: 2026-09-11

- After the first merge-readiness handoff, the owner explicitly rejected the agent's private-publication interpretation and established the governing intent: every brand built with the ShruggieTech brand skill is proudly displayed on the public brand website without exception. This supersedes the earlier zero-public-surface classification while leaving identity sources, geometry, copy, palette, typography, transformations, third-party ownership, endorsement, and service credit unchanged.
- The exact approved website surface set is `showcase-card`, `brand-landing-page`, `guideline-topics`, `downloads`, `registry-endpoints`, `public-metadata`, `structured-data`, and `social-preview`. Release tags and unrelated external deployments remain outside S028.
- Test-first evidence captured the old behavior: the new governed light-surface contract failed because `light.card` could not resolve, and the existing IHPRT site-preparation regression still expected private exclusion. The implementation now resolves `light.<role>` only through declared `light_surfaces`, sets IHPRT to public, binds the complete surface list, adds the canonical registry base and independent third-party notice, and requires the portfolio card to render on `#FFFFFF` with `#000000` text and the colored heart icon.
- Focused validation passes 53 brand-contract tests and 30 site-preparation tests. The website inventory and browser regressions now require exactly eight production brands, 16 desktop actions, eight mobile disclosures, complete no-script access, exact IHPRT copy and affiliation, the white governed card, and the colored heart source.
- The shared approval contract now rejects every completed Gate 2 brand whose showcase remains private or whose website surfaces are incomplete. Privacy remains valid during intake and pending review, but a completed skill-built brand cannot be hidden from the public brand website.
- The human-readable source contract changed only publication metadata, so the continuity record was refreshed without modifying identity artwork or derivative configuration. The resulting canonical source SHA-256 is `5ccb1f368bc70c250e4fe7a93528f93e6b21d77afc59c561c1b0e09953f5e7c8`, and the continuity record SHA-256 is `1363a6cce56a41a8443ef7ff871e0f1881a47a44c1c1aecd2426c41defd59598`.
- Full local parity passes 34 glyph checks, 5 package-release tests, 14 release-contract tests, 30 site-preparation tests, 4 continuity-audit tests, 53 brand-contract tests, 22 identity-continuity tests, 17 icon-suite tests, 58 pipeline tests, and Markdown policy. All eight production kits rebuild with zero reported verifier, glyph, affiliation, image-QC, PDF-QC, or pagination problems.
- Release certification verifies the existing nine v1.2.1 release assets. Release-tag inclusion remains outside S028, while the website-generated IHPRT download is present through its approved `downloads` surface.
- Site preparation publishes eight kits and ten reference documents. Lint and TypeScript pass, the static build emits 81 pages, all 11 production-origin and payload tests pass, and browser verification covers 76 HTML routes at desktop and mobile widths with zero WCAG 2.1 AA violations.
- Generated brand-site directories are now ignored through the slug-independent `site/public/*/` rule, preventing each future public brand from requiring a new repository-hygiene exception.
- The final cross-artifact analysis covers all 32 functional requirements, 10 success criteria, and 79 task records. The publication correction has complete implementation and verification coverage with no unresolved ambiguity, duplicate requirement, or constitution conflict.
