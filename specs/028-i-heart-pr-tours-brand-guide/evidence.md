...
- The Python 3.8 regression now uses the repository UTF-8 writer, and the raster integration test skips only when no renderer is measured. Hosted canonical proof generation is pinned to the exact approved Node 24.11.0 and resvg 2.6.2 toolchain instead of preferring an unrelated native renderer or floating the host runtime. Continuity validation retains the strict renderer, settings, proof, and comparison-evidence hashes from Gate 1.
- The first review-ledger head `f0c2366` proved the Python 3.8 fix on GitHub but exposed that `node-version: 24` had advanced the hosted runtime to Node 24.20.0. The strict proof gate correctly rejected that unapproved runtime before publication. The workflow now requests exact Node 24.11.0, matching the approved continuity record, rather than weakening proof comparison.
