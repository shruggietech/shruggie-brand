...
- Hosted heads `f0c2366` and `1940d32` proved the Python 3.8 fix and then isolated two independent proof-environment differences: a floating Node version and operating-system-specific PNG output. Both runs were rejected before publication. The final workflow retains Windows as the exact canonical host, exports its 32 hash-validated approved proofs as an ephemeral CI artifact, and requires Linux to verify every artifact hash against the Gate 1 record before applying the existing topology, edge-band, bounds, centroid, and color tolerances. No generated proof raster is committed, and local canonical-host validation remains byte-exact.
