...
- Hosted heads `f0c2366`, `1940d32`, and `2fb834a` proved the Python 3.8 fix and then isolated three proof-environment differences: a floating Node version, operating-system-specific PNG output, and a 16px proof whose entire visible mark legitimately falls inside the edge band. Each affected run was rejected before publication. The final workflow retains Windows as the exact canonical host, exports its 32 hash-validated approved proofs as an ephemeral CI artifact, and requires Linux to verify every artifact hash against the Gate 1 record before applying unchanged topology, edge-band, bounds, centroid, and color tolerances. When no non-edge interior pixels exist, color is measured across the shared silhouette instead of producing an undefined result. An exact Node 24.11.0 Linux-container reproduction passed the complete IHPRT proof matrix against the Windows artifact. No generated proof raster is committed, and local canonical-host validation remains byte-exact.
