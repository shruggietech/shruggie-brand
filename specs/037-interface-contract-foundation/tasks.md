...
- [x] T042 Trigger exactly one second Codex review round with `@Codex review`, then wait for CI and all second-round review results
- [x] T043 Address every actionable second-round comment, reply and resolve each thread, rerun affected and full gates, commit, push, and do not request a third review round
- [x] T044 Confirm every required CI check is green, every review is satisfied or explicitly non-actionable, and no unresolved thread remains; update `specs/037-interface-contract-foundation/evidence.md` and ping the owner for the final review and merge ritual
