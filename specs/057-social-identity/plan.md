...
8. Every brand has a `social_image_approval` that binds the assembled SVG and PNG to exact SHA-256 values. The pinned Node resvg renderer produces approved PNG bytes consistently across platforms. Existing Gate 2 brands also retain their full derivative-manifest approval. Core-tier vector checks omit the PNG when its optional renderer is unavailable.
