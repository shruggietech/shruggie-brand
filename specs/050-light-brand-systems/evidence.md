...
## Session 4: PR review correction

- Codex review of commit `846d035` identified a dark-guide/light-showcase gap: a `light.*` card could consume a generated light presentation without accessible light border tokens or complete light-palette validation. This is a source-generator defect, so the correction is in `skill/templates/`, not a generated kit.
- Light border, input, and sidebar-border tokens are now derived against every local light surface regardless of guide mode. A `light.*` showcase declaration also requires the complete, contrast-validated light palette even when the guide defaults to dark. The guide default and dark presentation are unchanged.
- Added regression cases for incomplete and low-contrast light showcase palettes and for 3:1 borders with a dark guide and light card. The brand contract suite passed 59 tests and the full pipeline suite passed with the pinned Node 24.11.0 identity renderer. Markdown policy, diff whitespace, and strict UTF-8/LF/no-BOM/mojibake checks passed for the changed files. The pushed correction will be subject to the repository's full CI build before handoff.
