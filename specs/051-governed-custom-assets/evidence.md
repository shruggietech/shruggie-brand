...
The second and final Codex review identified three more cases: non-public files under always-copied hosted trees, more than two expression cards clipped by one fixed PDF sheet, and mixed local/external CSS URLs in SVG attributes. Follow-up validation now rejects non-public sources in those trees before site copying, checks each CSS URL independently, and emits two-card expression PDF pages for unbounded inventory length. Focused contract and five-expression PDF pagination regressions passed; no further review request will be made.
